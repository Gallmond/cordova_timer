...
var klaxon = function(){
	...
	this.mediaLocation = cordova.file.applicationDirectory+"www/audio/alarm_sample.mp3";
	
	handy.alarmMedia = new Media(this.mediaLocation,function(){
		debug("media success");
	},function(err){
		debug("media error: ");
		debug(err.code);
	...
	this.sound = function(){
		...
		// show button
		handy.silenceButton.style.display = "";

		// if already playing do nothing else play
		if(this.alarmStatus==Media.MEDIA_STARTING || this.alarmStatus==Media.MEDIA_RUNNING){
			return true;
		}else{
			handy.alarmMedia.play();
		}
	};
	this.silence = function(){
		...
		// stop alarm
		handy.alarmMedia.stop();

		// hide button
		handy.silenceButton.style.display = "none";
...
var timer = function(_id, _form){
	...
	this.soundAlarm = function(_now){
		...
		if(navigator && navigator.vibrate){
			...
			alertText+= " (no navigator.vibrate)";	
		}
		debug(alertText);
	};
	...
	this.stopTimer = function(){
		...
		this.form.elements.hours. value = this.hh_initial;
		this.form.elements.minutes. value = this.mm_initial;
		this.form.elements.seconds. value = this.ss_initial;
...
function onDeviceReady() {
	...
	if(cordova.file.applicationDirectory){
		debug("applicationDirectory ready:");
		debug(cordova.file.applicationDirectory);
	}
	...
	// init klaxon
	handy.klaxon = new klaxon();

	// get silence button
	debug("attachSilenceButton()");
	attachSilenceButton();
...
function attachSilenceButton(){

	// get button
	handy.silenceButton = document.getElementById("alarm_silence_button");

	// hide button
	handy.silenceButton.style.display = "none";

	// attach function
	handy.silenceButton.addEventListener("click", handy.klaxon.silence);

	debug("attachSilenceButton finish");
	return true;
}
